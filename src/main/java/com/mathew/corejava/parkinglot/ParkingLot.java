...
public class ParkingLot {
  ...
  private ParkingSpace tryToPark(Vehicle vehicle) {
    ...
    if(size.equals(Vehicle.VehicleSize.LARGE)) {
      ...
      if(mediumSpace != null) {
        ...
        // Check large spaces --> if this is null, then no fitting spaces are
        // avaliable
      ...
      if(smallSpace != null) {
        ...
        if(mediumSpace != null) {
          ...
          // Check large spaces --> if this is null, then no appropriate spaces
          // are avaliable
          return findAvaliableSpaceWithType(type, parkingAvailableBySize.get(Vehicle.VehicleSize.LARGE));
        }
      }
  ...
  private ParkingSpace areSpacesTaken(ArrayList<ParkingSpace> parkingSpaces) {
    if(parkingSpaces == null) { return null; }

    for(ParkingSpace space : parkingSpaces) {
      if(!space.getIsTaken()) { return space; }
    }
    return null;
  }
