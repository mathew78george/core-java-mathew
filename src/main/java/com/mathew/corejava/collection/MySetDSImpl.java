package com.mathew.corejava.collection;

import java.util.Collection;
import java.util.Iterator;
import java.util.Set;

public class MySetDSImpl<E> implements Set<E> {

	private Object[] table;
	private static final int DEFAULT_CAPACITY = 3;
	private int count;

	public MySetDSImpl() {
		table = new Object[DEFAULT_CAPACITY];
		count = 0;
	}

	@Override
	public int size() {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public boolean isEmpty() {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean contains(Object o) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public Iterator<E> iterator() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Object[] toArray() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public <T> T[] toArray(T[] a) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public boolean add(E e) {
		if (count == table.length) {
			ensureCapacity();
		}
		if (count > 0 && isDuplicate(e)) {
			return false;
		}
		table[count++] = e;
		return true;
	}

	private void ensureCapacity() {
		int newsize = table.length * 2;
		Object[] newtab = new Object[newsize];
		for (int ii = 0; ii < table.length; ii++) {
			newtab[ii] = table[ii];
		}
		table = newtab;
	}

	private boolean isDuplicate(E e) {
		for (E element : (E[]) table) {
			if (e.equals(element) && e.hashCode() == element.hashCode()) {
				return true;
			}
		}
		return false;
	}

	@Override
	public boolean remove(Object o) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean containsAll(Collection<?> c) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean addAll(Collection<? extends E> c) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean retainAll(Collection<?> c) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean removeAll(Collection<?> c) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public void clear() {
		// TODO Auto-generated method stub

	}

	public static void main(String[] args) {
		MySetDSImpl<String> set = new MySetDSImpl<String>();
		set.add("ONE");
		set.add("TWO");
		set.add("ONE");
		set.add("FOUR");
		set.add("FIVE");
		set.add("ONE");
		set.add("EIGH");
		set.add("ONE");
		set.add("FOUR");
		set.add("SIX");
		System.out.println("Done----"+set);
	}

}
